...
class EinstellungenImportExport(QDialog):
    def __init__(self, configPath):
        ...
        self.checkboxTextListe = ["Allgemeine Einstellungen", "GDT-Einstellungen", "LANR/Lizenzschlüssel"]
        ...
        if not gdttoolsL.GdtToolsLizenzschluessel.lizenzErteilt(lizenzschluessel, self.configIni["Erweiterungen"]["lanr"], gdttoolsL.SoftwareId.DOSISGDT):
    ...
    def accept(self):
        if self.buttonBox.button(QDialogButtonBox.StandardButton.Ok).text() == "Importieren...":
            ...
            fd.setModal(True)
            fd.setNameFilters(["DosisGDT-Einstellungsdateien (*.ded)"])
            ...
            if fd.exec() == 1:
                ...
                if "Allgemein" in configImport.sections() or "GDT" in configImport.sections() or "Erweiterungen" in configImport.sections():
                    i=0
                    ...
                    try:
                        ...
                        self.done(1)
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von DosisGDT", "Die Einstellungen wurden erfolgreich importiert. Damit diese wirksam werden, muss DosisGDT neu gestartet werden.\nSoll DosisGDT neu gestartet werden?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von DosisGDT", "Fehler beim Importieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                        mb.exec()
                else:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von DosisGDT", "Die Datei " + datei + " ist keine gültige DosisGDT-Konfigurationsdatei", QMessageBox.StandardButton.Ok)
                    mb.exec()
        else:
            fd = QFileDialog(self)
            ...
            if fd.exec() == 1:
                ...
                dateiname = datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d%H%M%S") + "_DosisGdtEinstellungen.ded"
                ...
                try:
                    with open(datei, "w") as exportfile:
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Information, "Hinweis von DosisGDT", "Die gewünschten Einstellungen wurden erfolgreich unter dem Namen " + dateiname + " exportiert.", QMessageBox.StandardButton.Ok)
                        mb.exec()
                        self.done(1)
                except Exception as e:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von DosisGDT", "Fehler beim Exportieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                    mb.exec()
